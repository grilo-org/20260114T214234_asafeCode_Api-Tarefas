...
public class TarefasCrudClassFixture : IClassFixture<CustomWebApplicationFactory>
{
    private readonly HttpClient _httpClient;

    protected TarefasCrudClassFixture(CustomWebApplicationFactory factory)
    {
    ...
    protected async Task<HttpResponseMessage> DoPut(string method, object request = null!, string token = "", string culture = "en")
    {
        ChangeRequestCulture(culture);
        AuthorizeRequest(token);
